...
#doesn't run as is
#Attempted rewrite for new BERT2BERT model but swapped to using BART

#I will eventually rewrite this as I used the same function to train on the CNN Dataset and the fine tune data
#changing the data we're training meant some things got commented out
#Bad design on my part
# I should have had separate functions running after each other
...
df = df.drop(columns=demographic_fields) #Drop the category headers
...
class NewsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        demographic_info = torch.tensor(row[2:18].values.astype(float), dtype=torch.float)  #I worry that I'm losing info here
        return {
...
def train():
    for epoch in range(10):
        ...
        for batch in dataloader:
            ...
            # Debug Check shapes after reshaping
...
#torch.save(model.state_dict(), 'weights/CNN7.pth')

# Load the model weights
model.load_state_dict(torch.load('weights/CNN8.pt'))
model.eval()
